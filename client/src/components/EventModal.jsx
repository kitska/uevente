...
const EventModal = ({ show, onClose, onSubmit, form, onChange, setForm, updating = false, promocodes = false }) => {
				...
				{promocodes || (
					<div className='flex-1 space-y-8'>
						...
						<form className='grid grid-cols-1 gap-6 md:grid-cols-2'>
							...
							{/* Poster Input */}
							<div className="w-full flex flex-col md:flex-row md:items-start md:space-x-6 space-y-4 md:space-y-0">
								<div className="flex-1">
									<label className="block mb-1 text-sm font-medium text-gray-700">Event poster</label>
									<div className="flex items-center space-x-2">
										...
										<input
											...
											onChange={onChange}
											className="flex-1 p-2 text-gray-700 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
										/>
									</div>
								</div>
							</div>
							{previewSrc && (
									<div className="w-40 h-28 border border-gray-300 rounded-md shadow-sm overflow-hidden flex items-center justify-center">
										<img
											src={previewSrc}
											alt="Preview"
				...
				{updating && (
					<div className='flex flex-col w-full max-w-xs space-y-6'>
						<div className='flex justify-between'>
							<h4 className={`text-3xl font-semibold text-gray-800`}>Promocodes</h4>
							{promocodes && (<button onClick={onClose} className='px-4 py-2 text-gray-500 rounded-md cursor-pointer hover:text-gray-700'>
								Cancel
							</button>)}
						</div>
